...
import { Flow_Screenshots_Insert_Input, InputMaybe } from '@/graphql/generated'
...
type Props = {
  ...
  isQueued: boolean
  title?: string
  item?: Flow_Screenshots_Insert_Input
  index?: number
}

const UploadTabScreenshotItem: FC<Props> = ({
  image,
  uploadPercentage,
  isQueued,
  title,
  ...
  const removeHandler = () => {
    if (index === 0 || index) dispatch(removeScreenshot(index))
  }
      ...
      <UploadFlatInput
        ...
        handleBlur={(value) => {
          if (item && (index === 0 || index)) {
            dispatch(updateScreenshotTitle({ index, title: value }))
          }
        }}
        isBold={false}
        autofocus={false}
...
export default UploadTabScreenshotItem
