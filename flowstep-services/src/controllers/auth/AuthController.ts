...
export class AuthController {
  ...
  public googleUrl = async (req: Request, res: Response): Promise<void> => {
    try {
      serviceResponseHandler(res, await this.authService.goggleUrl())
    } catch (error: any) {
      res.status(500).send({ message: error.message })
    }
  }
  public googleCallback = async (
    ...
    try {
      ...
      serviceResponseHandler(
        res,
        await this.authService.googleCallback(code as string)
      )
    } catch (error: any) {
      res.status(500).send({ message: error.message })
    }
  ...
  public checkEmail = async (req: Request, res: Response) => {
    try {
      const { email } = req.body.input
      serviceResponseHandler(res, await this.authService.checkEmail(email))
    } catch (error: any) {
      res.status(500).send({ message: error.message })
    }
  }
